...
// =============================================================================
// Polybar Output Golden Tests
// =============================================================================

#[test]
fn golden_polybar_empty() {
    let formatter = OutputFormatter::with_defaults();
    let now = local_from_utc(reference_time());

    let output = formatter.format_polybar_at(&[], "No meetings", now);

    insta::assert_snapshot!("polybar_empty", output);
}

#[test]
fn golden_polybar_single_upcoming() {
    let now_utc = reference_time();
    let now_local = local_from_utc(now_utc);
    let meetings = vec![sample_meeting(now_utc, 15, "Team Standup")];

    let formatter = OutputFormatter::with_defaults();
    let output = formatter.format_polybar_at(&meetings, "No meetings", now_local);

    insta::assert_snapshot!("polybar_single_upcoming", output);
}

#[test]
fn golden_polybar_ongoing() {
    let now_utc = reference_time();
    let now_local = local_from_utc(now_utc);
    let meetings = vec![ongoing_meeting(now_utc, "Sprint Review")];
    ...
    insta::assert_snapshot!("polybar_ongoing", output);
}

#[test]
fn golden_polybar_all_day_only() {
    let now_utc = reference_time();
    let now_local = local_from_utc(now_utc);
    let meetings = vec![all_day_meeting(now_utc, "Team Offsite")];

    let formatter = OutputFormatter::with_defaults();
    let output = formatter.format_polybar_at(&meetings, "No meetings", now_local);

    insta::assert_snapshot!("polybar_all_day_only", output);
}

#[test]
fn golden_polybar_prefers_timed_over_allday() {
    let now_utc = reference_time();
    let now_local = local_from_utc(now_utc);
    let meetings = vec![
    ...
    insta::assert_snapshot!("polybar_prefers_timed", output);
}
