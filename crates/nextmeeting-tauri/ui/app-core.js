...
export function parseClockOnDate(baseDate, hhmm) {
...
export function parseMeetingDate(value, fallbackBaseDate, fallbackTime) {
  if (value) {
...
export function meetingStartDate(meeting, baseDate = new Date()) {
  return parseMeetingDate(meeting.startAt, baseDate, meeting.startTime);
}

export function meetingEndDate(meeting, baseDate = new Date()) {
...
export function truncateLabel(value, maxLength = 34) {
...
export function joinButtonLabel(meeting, prefix = "Join") {
...
export function buildMeetingDetailItems(meeting) {
  const items = [];
...
function appendMeetingDetailRows(doc, container, items) {
...
export function buildTimelineMeetings(meetings, rangeStart, rangeEnd) {
...
export function createDashboardApp({
  ...
  const timelineNode = doc.querySelector("#timeline");
  const meetingListNode = doc.querySelector("#meetingList");
  const actionListNode = doc.querySelector("#actionList");
  const sourceBadgeNode = doc.querySelector("#sourceBadge");
  const todayTitleNode = doc.querySelector("#todayTitle");
  const heroTitleNode = doc.querySelector("#heroTitle");
  const heroMetaNode = doc.querySelector("#heroMeta");
  const statusLineNode = doc.querySelector("#statusLine");
  const joinNowButtonNode = doc.querySelector("#joinNowButton");
  const createMeetingButtonNode = doc.querySelector("#createMeetingButton");
  const heroMeetingDetailsNode = doc.querySelector("#heroMeetingDetails");
  const heroMeetingDetailsSummaryNode = doc.querySelector("#heroMeetingDetailsSummary");
  const heroMeetingDetailsContentNode = doc.querySelector("#heroMeetingDetailsContent");
  const panelNode = doc.querySelector(".panel");
  const isMac = win.navigator.platform.toUpperCase().includes("MAC");

  function renderTimeline(meetings = []) {
    const currentNow = now();
    const currentHour = currentNow.getHours();
    const rangeStart = new Date(currentNow);
    ...
    const meetingsContainer = doc.createElement("div");
  ...
  function createMeetingDetails(meeting) {
    ...
    appendMeetingDetailRows(doc, content, items);
  ...
  function renderHeroDetails(meeting) {
    ...
    heroMeetingDetailsContentNode.textContent = "";
    appendMeetingDetailRows(doc, heroMeetingDetailsContentNode, items);
  ...
  function renderMeetings(meetings) {
    ...
    for (const meeting of meetings.slice(0, 4)) {
      ...
      const header = meeting.joinUrl ? doc.createElement("button") : doc.createElement("div");
      ...
      const dayP = doc.createElement("p");
      ...
      const timeP = doc.createElement("p");
      ...
      const serviceDiv = doc.createElement("div");
  ...
  function setConnectionHealth(source) {
    const dot = sourceBadgeNode.querySelector(".health-dot") || doc.createElement("span");
  ...
  async function main() {
    ...
    setIntervalFn(refreshDashboard, REFRESH_INTERVAL_MS);
  }
