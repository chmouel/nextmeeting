#!/usr/bin/env python3  # legacy archive
...
class NotificationManager:
    def __init__(self, args: argparse.Namespace):
        ...
        self.replace_ids_path = Path(args.cache_dir) / "replace_ids.json"
    ...
    def notify_if_needed(
        ...
        self._send_with_replace(
            title, body, key=uuid, urgency_override=urgency_override
        )
    ...
    def _send_with_replace(
        self, title: str, body: str, key: str, urgency_override: Optional[str] = None
    ) -> None:
        urgency = urgency_override or getattr(self.args, "notify_urgency", None)
        expiry_min = getattr(self.args, "notify_expiry", 0)
        with_actions = getattr(self.args, "notify_actions", False)
        icon = os.path.expanduser(self.args.notify_icon)
        # Prefer DBus when actions requested, else try notify-send with replace-id
        if with_actions:
            try:
                # Send via DBus (returns id)
                new_id = asyncio.run(
                    _send_notification_with_actions(
                        title, body, icon, urgency, expiry_min
                    )
                )
                if new_id:
                    self._set_replace_id(key, new_id)
                return
            except Exception:
                pass

        # Try CLI path with replace-id support (notify-send)
        cmd = [NOTIFY_PROGRAM, "-i", icon]
        # urgency
        if urgency:
            cmd.extend(["-u", urgency])
        # expiry
        if expiry_min > 0:
            cmd.extend(["-t", str(expiry_min * 60 * 1000)])
        elif expiry_min < 0:
            cmd.extend(["-t", str(NOTIFY_MIN_BEFORE_EVENTS * 60 * 1000)])
        # replace-id if known
        prev_id = self._get_replace_id(key)
        if prev_id:
            cmd.extend(["--replace-id", str(prev_id)])
        # ask libnotify to print id if available
        cmd.append("--print-id")
        ...
        try:
            ...
            if res.returncode == 0 and res.stdout:
                ...
                # notify-send prints the id (uint32)
                if out.isdigit():
                    self._set_replace_id(key, int(out))
                return
        except Exception:
            pass
        # Fallback plain notify-send
        subprocess.call([NOTIFY_PROGRAM, "-i", icon, title, body])

    def _get_replace_id(self, key: str) -> Optional[int]:
        try:
            if not self.replace_ids_path.exists():
                return None
            data = json.loads(self.replace_ids_path.read_text())
            rid = data.get(key)
            return int(rid) if rid is not None else None
        except Exception:  # noqa: BLE001
            return None

    def _set_replace_id(self, key: str, rid: int) -> None:
        try:
            data = {}
            if self.replace_ids_path.exists():
                try:
                    data = json.loads(self.replace_ids_path.read_text())
                except Exception:  # noqa: BLE001
                    data = {}
            data[key] = int(rid)
            self.replace_ids_path.write_text(json.dumps(data))
        except Exception:  # noqa: BLE001
            pass
    ...
    def notify_morning_agenda_if_needed(self, meetings: list[Meeting]):
        ...
        body = "\n".join(lines)
        # Use replace-id keyed per day to update in place
        # Temporarily override expiry_min by calling notify-send fallback with replace-id
        self._send_with_replace(title="Today's meetings", body=body, key=today_key)
...
def notify(
    ...
    # Dynamic urgency: escalate to critical within threshold
    urgency_override = None
    threshold = getattr(args, "notify_critical_within", None)
    if isinstance(threshold, int) and threshold >= 0:
        try:
            mins = int(
                max(0, (start_date - datetime.datetime.now()).total_seconds() // 60)
            )
            if mins <= threshold:
                current = getattr(args, "notify_urgency", None) or "normal"
                if current != "critical":
                    urgency_override = "critical"
        except Exception:  # noqa: BLE001
            pass
    notification_manager.notify_if_needed(title, start_date, end_date, urgency_override)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        help="Auto-escalate urgency to 'critical' when the meeting starts within N minutes",
    )
    parser.add_argument(
        "--notify-actions",
        action="store_true",
        help="Add Join/Copy/Snooze buttons in notifications (swaync supported)",
    )
    parser.add_argument(
        "--snooze",
        type=int,
    ...
    parser.add_argument(
        "--actions-daemon",
        action="store_true",
        help="Run a small DBus listener to handle notification action clicks",
    )
    parser.add_argument(
        "--actions-daemon-timeout",
        type=int,
        default=0,
        help="Exit actions daemon automatically after N minutes (0 = never)",
    )
...
def main():
    ...
    # Optional: run actions daemon mode
    if getattr(args, "actions_daemon", False):
        try:
            asyncio.run(run_actions_daemon(args))
...
# --- Notification actions (swaync) support ------------------------------------
async def _send_notification_with_actions(
    ...
    """Send a notification with actions using DBus (freedesktop).

    Requires dbus-next. If unavailable or sending fails, raises and caller may
    fall back to notify-send.
    """
    try:
        # Lazy import to avoid hard dep when not used
        from dbus_next.aio import MessageBus  # type: ignore  # noqa: PLC0415
        from dbus_next import Message  # type: ignore  # noqa: PLC0415
        from dbus_next.signature import Variant  # type: ignore  # noqa: PLC0415
    except Exception:  # noqa: BLE001
    ...
    # Map urgency to spec: 0 low, 1 normal, 2 critical
    level_map = {"low": 0, "normal": 1, "critical": 2}
    hints = {
        "urgency": Variant("y", level_map.get((urgency or "normal"), 1)),
        "category": Variant("s", "im.received"),
        "desktop-entry": Variant("s", "nextmeeting"),
    }
    timeout_ms = 0
    if expiry_min > 0:
        timeout_ms = expiry_min * 60 * 1000
    elif expiry_min < 0:
        timeout_ms = NOTIFY_MIN_BEFORE_EVENTS * 60 * 1000
    ...
    # Return the notification id so caller can reuse it
    notif_id = None
    try:
        if reply and reply.body:
            notif_id = int(reply.body[0])
    except Exception:
        notif_id = None
    await bus.wait_for_disconnect()  # Close quickly; we only send, not listen
    return notif_id


async def run_actions_daemon(args: argparse.Namespace) -> None:
    """Listen for notification action clicks and perform actions.

    Supported actions:
      - join: open next meeting link
      - copy: copy next meeting link
      - snooze: snooze notifications for 5 minutes
    """
    try:
        from dbus_next.aio import MessageBus  # type: ignore  # noqa: PLC0415
        from dbus_next import Message  # type: ignore  # noqa: PLC0415
    except Exception:  # noqa: BLE001
        print("dbus-next is required for --actions-daemon", file=sys.stderr)
        raise
        ...
        if (
            ...
            try:
                ...
                if action == "join":
                    ...
                    NotificationManager(args).set_snooze(5)
            except Exception:
                pass
    ...
    # Add match rule for ActionInvoked signals
    await bus.call(
        Message(
    ...
    async def _shutdown_after_timeout():
        if getattr(args, "actions_daemon_timeout", 0) > 0:
            await asyncio.sleep(args.actions_daemon_timeout * 60)
...
def _join_next(args: argparse.Namespace) -> None:
    fetcher = MeetingFetcher()
    try:
        meetings = fetcher.fetch_meetings(args)
        meeting = get_next_meeting(
            meetings, getattr(args, "skip_all_day_meeting", False)
        )
        if not meeting:
            return
        sanitized = sanitize_meeting_link(meeting.meet_url)
        url = (sanitized.url if sanitized else None) or meeting.calendar_url
        open_url(url, args)
    except Exception:
        pass


def _copy_next(args: argparse.Namespace) -> None:
    fetcher = MeetingFetcher()
    try:
        meetings = fetcher.fetch_meetings(args)
        meeting = get_next_meeting(
