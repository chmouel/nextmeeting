...
def make_meeting(title="Test Meeting", start_time=None, end_time=None, calendar_url=None, meet_url=None):
...
@pytest.mark.parametrize(("tsv", "title"), [
    ("2025-07-04 10:00 2025-07-04 11:00 https://calendar.google.com/test https://meet.google.com/test Test Meeting", "Test Meeting"),
])
...
def test_ret_events_upcoming_and_ongoing():
    args = Namespace(today_only=False, waybar=False, skip_all_day_meeting=False, google_domain=None, notify_min_before_events=5, notify_min_color="#FF5733", notify_min_color_foreground="#F4F1DE", waybar_show_all_day_meeting=False)
    rets, cssclass = cli.ret_events([FAKE_MEETING, FAKE_MEETING_ONGOING], args)
...
def test_notify(monkeypatch, tmp_path):
    args = Namespace(cache_dir=tmp_path, notify_expiry=0, notify_icon="/tmp/icon.svg", notify_min_before_events=5, debug=False)
    ...
    cli.notify("Test", datetime.datetime.now(), datetime.datetime.now() + datetime.timedelta(minutes=1), args)
    assert called.get('called')

# Test MeetingFetcher abstraction
def test_meetingfetcher_fetch_meetings(monkeypatch):
    fetcher = cli.MeetingFetcher()
    dummy_args = Namespace(gcalcli_cmdline="echo '2025-07-04 10:00 2025-07-04 11:00 https://calendar.google.com/test https://meet.google.com/test Test Meeting'", debug=False)
    monkeypatch.setattr(cli, "process_lines", lambda lines: [FAKE_MEETING])
...
def test_all_day_meeting_detection_and_skip():
    all_day = make_meeting(
        ...
        end_time=(datetime.datetime.now() + datetime.timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0),
    )
    assert all_day.is_all_day
    args = Namespace(today_only=False, waybar=False, skip_all_day_meeting=True, google_domain=None, notify_min_before_events=5, notify_min_color="#FF5733", notify_min_color_foreground="#F4F1DE", waybar_show_all_day_meeting=False)
    rets, cssclass = cli.ret_events([all_day, FAKE_MEETING], args)
    # Only FAKE_MEETING should be present
    assert all("All Day" not in r for r in rets)

# Test elipsis function
def test_elipsis():
    s = "<b>Hello</b> World!" + "x" * 60
    result = cli.elipsis(s, 20)
    assert result.endswith("...")
    assert "<b>" not in result
    assert len(result) <= 20
    s2 = "Short string"
    assert cli.elipsis(s2, 50) == s2

# Test pretty_date for today, tomorrow, negative deltas
def test_pretty_date_cases():
    args = Namespace(notify_min_color="#FF5733", notify_min_color_foreground="#F4F1DE", waybar=True)
    now = datetime.datetime.now()
    # Today, in 10 minutes
...
def test_get_next_helpers():
    ...
    all_day = make_meeting(
        title="All Day Meeting",
        start_time=datetime.datetime.now().replace(hour=0, minute=0, second=0, microsecond=0),
        end_time=(datetime.datetime.now() + datetime.timedelta(days=1, hours=1)).replace(hour=1, minute=0, second=0, microsecond=0),
    )
...
def test_cli_output_waybar(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(cli, "parse_args", lambda: Namespace(
        waybar=True, waybar_show_all_day_meeting=True, calendar=None, gcalcli_cmdline=cli.GCALCLI_CMDLINE, cache_dir=tmp_path, max_title_length=50, notify_icon="/tmp/icon.svg", notify_expiry=0, skip_all_day_meeting=False, today_only=False, google_domain=None, notify_min_before_events=5, notify_min_color="#FF5733", notify_min_color_foreground="#F4F1DE", open_meet_url=False, debug=False
    ))
    cli.main()
    assert out['obj']['text']
...
def test_cli_output_non_waybar(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(cli, "MeetingFetcher", DummyFetcher)
    monkeypatch.setattr(cli, "parse_args", lambda: Namespace(
        waybar=False, waybar_show_all_day_meeting=False, calendar=None, gcalcli_cmdline=cli.GCALCLI_CMDLINE, cache_dir=tmp_path, max_title_length=50, notify_icon="/tmp/icon.svg", notify_expiry=0, skip_all_day_meeting=False, today_only=False, google_domain=None, notify_min_before_events=5, notify_min_color="#FF5733", notify_min_color_foreground="#F4F1DE", open_meet_url=False, debug=False
    ))
    cli.main()
